...
TOKEN_LEAK_PATTERN = re.compile(
    r"(?:GLBPROTECTED|GLBGLOSSARY|\{\{[^{}]+\}\})", re.IGNORECASE)
NON_TRANSLATABLE_TEMPLATE_KEYS = {"sprache", "marke", "code"}
PROTECTED_VALUE_FIELDS = ("key", "wert", "einheit", "referenzMin",
                          "referenzMax", "status", "trend", "verlauf")
...
def translate_report(report_data: dict, target_language: str) -> dict:
    ...
    Args:
        report_data: Wert für ``report_data``.
        target_language: ISO-Sprachcode der gewünschten Zielsprache.
    ...
    if target not in settings.GLOBI_TRANSLATION_LANGUAGES:
        raise ValidationError(
            "Die gewünschte Zielsprache ist nicht freigegeben.")
    if not report_data.get("istDruckbar") or report_data.get("reviewWerte", 0) > 0:
        raise ValidationError(
            "Ein Bericht darf erst ohne offene Reviews übersetzt werden.")
    ...
    template["bericht"] = _translate_structure(
        template.get("bericht", {}), translator, glossary)
    template["statusLabels"] = _translate_structure(
        template.get("statusLabels", {}), translator, glossary)
    template["prioritaetLabels"] = _translate_structure(
        template.get("prioritaetLabels", {}), translator, glossary)
    template["sprache"] = target

    for key in ("gesamtstatus", "gesamttext", "disclaimer"):
        translated[key] = _translate_text(
            translated.get(key, ""), translator, glossary)

    for item in translated.get("werte", []):
        for key in ("name", "gruppe", "erklaerung", "hinweis"):
            item[key] = _translate_text(
                item.get(key, ""), translator, glossary)

    for item in translated.get("kategorien", []):
        item["name"] = _translate_text(
            item.get("name", ""), translator, glossary)

    for item in translated.get("empfehlungen", []):
        item["titel"] = _translate_text(
            item.get("titel", ""), translator, glossary)
        item["text"] = _translate_text(
            item.get("text", ""), translator, glossary)

    translated["fragen"] = [_translate_text(
        item, translator, glossary) for item in translated.get("fragen", [])]
    translated["quellen"] = [
        {
            **item,
...
def _load_argos_translator(source_language: str, target_language: str):
    ...
    Args:
        source_language: Wert für ``source_language``.
        target_language: ISO-Sprachcode der gewünschten Zielsprache.
    """
    try:
        ...
        raise TranslationUnavailable(
            "Argos Translate ist lokal noch nicht installiert.") from error

    installed_languages = argostranslate.translate.get_installed_languages()
    source = next(
        (item for item in installed_languages if item.code == source_language), None)
    target = next(
        (item for item in installed_languages if item.code == target_language), None)
    if source is None or target is None:
        raise TranslationUnavailable(
            "Das benötigte lokale Argos-Sprachmodell ist nicht installiert.")

    translation = source.get_translation(target)
    if translation is None:
        raise TranslationUnavailable(
            f"Für {source_language} → {target_language} ist kein lokaler Übersetzungsweg verfügbar.")
    return translation
...
def _load_glossary(source_language: str, target_language: str) -> dict[str, str]:
    ...
    Returns:
        Rückgabewert vom Typ ``dict[str, str]``.
    """
...
def _translate_structure(value: Any, translator, glossary: dict[str, str], key: str = "") -> Any:
    ...
    Args:
        ...
        translator: Wert für ``translator``.
        glossary: Wert für ``glossary``.
        key: Stabiler fachlicher oder technischer Schlüssel.

    Returns:
        Rückgabewert vom Typ ``Any``.
    """
    if isinstance(value, dict):
        return {
            item_key: item_value if item_key in NON_TRANSLATABLE_TEMPLATE_KEYS else _translate_structure(
                item_value, translator, glossary, item_key)
...
def _translate_text(text: str, translator, glossary: dict[str, str]) -> str:
    ...
    for start, end, replacement in protected_segments:
        if start > cursor:
            result.append(_translate_plain_segment(
                value[cursor:start], translator))
...
def _collect_protected_segments(text: str, glossary: dict[str, str]) -> list[tuple[int, int, str]]:
    ...
    Returns:
        Rückgabewert vom Typ ``list[tuple[int, int, str]]``.
    """
    segments: list[tuple[int, int, str]] = [(match.start(), match.end(
    ), match.group(0)) for match in PROTECTED_PATTERN.finditer(text)]
    ...
    for original_value, translated_value in zip(original_values, translated_values, strict=True):
        for field in PROTECTED_VALUE_FIELDS:
            if original_value.get(field) != translated_value.get(field):
                raise TranslationUnavailable(
                    f"Das geschützte Berichtsfeld '{field}' wurde verändert.")

    if len(original.get("empfehlungen", [])) != len(translated.get("empfehlungen", [])):
        raise TranslationUnavailable(
            "Die Übersetzung hat Empfehlungen entfernt oder ergänzt.")
    if len(original.get("fragen", [])) != len(translated.get("fragen", [])):
        raise TranslationUnavailable(
            "Die Übersetzung hat Fragen entfernt oder ergänzt.")

    serialized = json.dumps(translated, ensure_ascii=False)
    if TOKEN_LEAK_PATTERN.search(serialized):
        raise TranslationUnavailable(
            "Die Übersetzung enthält nicht aufgelöste Schutzmarkierungen.")
